...
/**
 * 背景阴影叠加布局，将需要的空间布局在其中就可以了
 * 原文链接:https://github.com/Dsiner/UIUtil
 */
public class ShadowLayout extends FrameLayout {
    ...
    public ShadowLayout(final Context context, final AttributeSet attrs, final int defStyleAttr) {
        ...
        try {
            setIsShadowed(typedArray.getBoolean(R.styleable.ShadowLayout_shadowed, true));
            setShadowRadius(
                    typedArray.getDimension(
                            R.styleable.ShadowLayout_shadowRadius, DEFAULT_SHADOW_RADIUS
                    )
            );
            setShadowDistance(
                    typedArray.getDimension(
                            R.styleable.ShadowLayout_shadowDistance, DEFAULT_SHADOW_DISTANCE
                    )
            );
            setShadowAngle(
                    typedArray.getInteger(
                            R.styleable.ShadowLayout_shadowAngle, (int) DEFAULT_SHADOW_ANGLE
                    )
            );
            setShadowColor(
                    typedArray.getColor(
                            R.styleable.ShadowLayout_shadowColor, DEFAULT_SHADOW_COLOR
                    )
            );
        } finally {
            typedArray.recycle();
