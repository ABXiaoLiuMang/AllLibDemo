...
public abstract class AttachPopupView extends BasePopupView {
    ...
    protected void initPopupContent() {
        ...
        defaultOffsetY = popupInfo.offsetY == 0 ? SizeUtils.dp2px(4) : popupInfo.offsetY;
        defaultOffsetX = popupInfo.offsetX == 0 ? SizeUtils.dp2px(0) : popupInfo.offsetX;
        ...
        if (!popupInfo.hasShadowBg) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                ...
                attachPopupContainer.setElevation(SizeUtils.dp2px(10));
            } else {
                //优先使用implView的背景
    ...
    // 弹窗显示的位置不能超越Window高度
    float maxY = ScreenUtils.getScreenHeight();
    ...
    protected void doAttach() {
        ...
        if (popupInfo.touchPoint != null) {
            ...
            if (isTallerThanWindowHeight) {
                isShowUp = popupInfo.touchPoint.y > ScreenUtils.getScreenHeight() / 2;
            } else {
                isShowUp = false;
            }
            isShowLeft = popupInfo.touchPoint.x < ScreenUtils.getScreenWidth() / 2;
            ...
            if (isShowUpToTarget()) {
                if (getPopupContentView().getMeasuredHeight() > popupInfo.touchPoint.y) {
                    ...
                    params.height = (int) (popupInfo.touchPoint.y - StatusBarUtil.getStatusBarHeight(getContext()));
                    getPopupContentView().setLayoutParams(params);
                }
            } else {
                if (getPopupContentView().getMeasuredHeight() + popupInfo.touchPoint.y > ScreenUtils.getScreenHeight()) {
                    ViewGroup.LayoutParams params = getPopupContentView().getLayoutParams();
                    params.height = (int) (ScreenUtils.getScreenHeight() - popupInfo.touchPoint.y);
                    getPopupContentView().setLayoutParams(params);
                }
            }
            ...
            if (isTallerThanWindowHeight) {
                ...
                isShowUp = centerY > ScreenUtils.getScreenHeight() / 2;
            } else {
                isShowUp = false;
            }
            isShowLeft = centerX < ScreenUtils.getScreenWidth() / 2;

            //修正高度，弹窗的高有可能超出window区域
            if (isShowUpToTarget()) {
                if (getPopupContentView().getMeasuredHeight() > rect.top) {
                    ViewGroup.LayoutParams params = getPopupContentView().getLayoutParams();
                    params.height = rect.top - StatusBarUtil.getStatusBarHeight(getContext());
                    getPopupContentView().setLayoutParams(params);
                }
            } else {
                if (getPopupContentView().getMeasuredHeight() + rect.bottom > ScreenUtils.getScreenHeight()) {
                    ViewGroup.LayoutParams params = getPopupContentView().getLayoutParams();
                    params.height = ScreenUtils.getScreenHeight() - rect.bottom;
                    getPopupContentView().setLayoutParams(params);
                }
            }
